...
from knowledge_bert.tokenization_origin import BasicTokenizer, whitespace_tokenize
import csv
...
special_tokens = {"[unused1]", "[unused2]", "[unused3]", "[unused4]", "[unused5]"}
...
class QAExample(object):
    ...
    def __init__(self,
        ...
        self.entity_ids = entity_ids  # 02 Add
        self.entity_masks = entity_masks  # 02 Add
...
class InputFeatures(object):
    ...
    def __init__(self,
        ...
        self.tokens = tokens
        self.entity_ids = entity_ids  # 04 Add
        self.entity_masks = entity_masks  # 04 Add
...
def read_qa_examples(input_file, is_training, version_2_with_negative):
    a = 0
    quest = []
    abstract = []
    entity_ids = []   # 05 Add
    ...
    with open(input_file, encoding='utf-8') as f:
        ...
        for content in text:
            for key in content:
                if key in question_box.keys():  # 06 Change
                    abstract.append(content["Abstract"])

                    # 07 Add
                    # if content["am_id"] is None:
                    #     entity_graph_embeddings.append([0] * 100)
                    # else:
                    entity_ids.append(content["Annotation"])  # list
                    entity_masks.append([])
                    # End Add

                    quest.append(question_box[key])
    ...
    while i < total:
        ...
        # node = abstract_node[i]  # 08 Add
        ent_ids = entity_ids[i]
        ent_masks = entity_masks[i]  # == []
        start_position = None
        end_position = None
        orig_answer_text = None
        is_impossible = False
        ...
        example = QAExample(
            ...
            entity_ids=ent_ids,  # 09 Add
            entity_masks=ent_masks,
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    entity2id = {}
    with open("preprocess/acekg/entity2id.txt") as fin:
        fin.readline()
        for line in fin:
    ...
    for (example_index, example) in enumerate(examples):
        ...
        annotation_ids = example.entity_ids
        ent_counter = 0
        ent_ids = []  # 09 Add
        ent_masks = []
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            for i in range(doc_span.length):
                ...
                if all_doc_tokens[split_token_index] in special_tokens:
                    ent_ids.append(entity2id["Q" + str(annotation_ids[ent_counter])])
                    ent_masks.append(1)
                    ent_counter += 1
                else:
                    ent_ids.append(-1)
            ...
            assert len(ent_ids) == max_seq_length
            assert len(ent_masks) == max_seq_length
            ...
            unique_id += 1
            break
        # assert ent_counter == len(annotation_ids), "{} is not equal to {}".format(ent_counter, len(annotation_ids))
    return features
